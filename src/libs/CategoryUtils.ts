...
function formatItemizedReceiptsOverText(translate: LocaleContextProps['translate'], policy: Policy, categoryMaxAmountNoItemizedReceipt?: number | null) {
    ...
    if (isAlwaysSelected) {
        return translate(`workspace.rules.categoryRules.itemizedReceiptsOverList.always`);
    }

    if (isNeverSelected) {
        return translate(`workspace.rules.categoryRules.itemizedReceiptsOverList.never`);
    }
    ...
    return translate(`workspace.rules.categoryRules.itemizedReceiptsOverList.default`, {
        defaultAmount: convertToShortDisplayString(maxExpenseAmountToDisplay, policy?.outputCurrency ?? CONST.CURRENCY.USD),
    });
}
...
export {
    ...
    formatItemizedReceiptsOverText,
