...
import {formatDefaultTaxRateText, formatItemizedReceiptsOverText, formatRequireReceiptsOverText, getCategoryApproverRule, getCategoryDefaultTaxRate} from '@libs/CategoryUtils';
...
function CategorySettingsPage({
    ...
    const itemizedReceiptsOverText = useMemo(() => {
        if (!policy) {
            return '';
        }
        return formatItemizedReceiptsOverText(translate, policy, policyCategory?.maxAmountNoItemizedReceipt);
                    ...
                    {!!policy?.areRulesEnabled && (
                        <>
                            ...
                            <OfflineWithFeedback pendingAction={policyCategory.pendingFields?.maxAmountNoItemizedReceipt}>
                                <MenuItemWithTopDescription
                                    title={itemizedReceiptsOverText}
                                    description={translate(`workspace.rules.categoryRules.itemizedReceiptsOver`)}
                                    onPress={() => {
                                        Navigation.navigate(ROUTES.WORKSPACE_CATEGORY_ITEMIZED_RECEIPTS_OVER.getRoute(policyID, policyCategory.name));
                                    }}
                                    shouldShowRightIcon
                                />
                            </OfflineWithFeedback>
